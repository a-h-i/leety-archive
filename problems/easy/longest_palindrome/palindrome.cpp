...
int Solution::longestPalindrome(string s) {
  ...
  for (auto &charFrequency : charFrequencyMap) {
    if (charFrequency.second % 2 == 0) {
      ...
      charFrequency.second =
          0; // we have taken all repetition of this character
  ...
  // now we can add a single character in the middle that only has one
  // repetition left, doesn't matter if it was selected before or not
  for (auto &charFrequency : charFrequencyMap) {
